...
class AgentArchitect(Agent):
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if "dataset" in task.description.lower() or "veri seti" in task.description.lower():
                path = task.context.get("path")
                if path:
                    await self.knowledge.index_dataset(path)
                    return TaskResult(task_id=task.id, status=TaskStatus.SUCCESS, output=f"Successfully indexed dataset at {path}. My context awareness is increasing.")

            # Check if directory indexing is requested
            if "index" in task.description.lower() or "indeksle" in task.description.lower():
                path = task.context.get("path")
                if path:
                    await self.knowledge.index_directory(path)
                    return TaskResult(task_id=task.id, status=TaskStatus.SUCCESS, output=f"Successfully indexed library at {path}")

            # General architectural task
            messages = self._build_messages(task.description)
