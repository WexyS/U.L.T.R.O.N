...
class ClonerAgent(Agent):
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if not url and "http" in task.description:
                import re
                match = re.search(r'https?://\S+', task.description)
                if match: url = match.group(0)

            if not url:
                return TaskResult(task_id=task.id, status=TaskStatus.FAILED, error="No URL provided for cloning")
            ...
            if visual_data_result.status != TaskStatus.SUCCESS:
                return TaskResult(task_id=task.id, status=TaskStatus.FAILED, error=f"Visual analysis failed: {visual_data_result.error}")
