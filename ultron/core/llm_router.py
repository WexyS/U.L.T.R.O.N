...
from ultron.core.error_analyzer import ErrorAnalyzer
from ultron.core.connectivity import ConnectivityManager
from ultron.providers.base import ProviderStats
...
class LLMRouter:
    ...
    def __init__(
        ...
        try:
            from ultron.providers.brain_provider import BrainProvider
    ...
    def enable_groq(self, api_key: str, model: str = "fast"):
        ...
        if api_key and api_key.startswith("gsk_"):
            from ultron.providers.all_providers import GroqProvider
    ...
    def enable_gemini(self, api_key: str, model: str = "gemini-2.0-flash"):
        ...
        if api_key and api_key.startswith("AIza"):
            from ultron.providers.all_providers import GeminiProvider
    ...
    def enable_cloudflare(self, api_key: str, account_id: str, model: str = "@cf/meta/llama-3.1-8b-instruct"):
        ...
        if api_key and account_id:
            from ultron.providers.all_providers import CloudflareProvider
    ...
    def enable_together(self, api_key: str, model: str = "Qwen/Qwen2.5-72B-Instruct-Turbo"):
        ...
        if api_key:
            from ultron.providers.all_providers import TogetherProvider
    ...
    def enable_huggingface(self, api_key: str, model: str = "Qwen/Qwen2.5-72B-Instruct"):
        ...
        if api_key:
            from ultron.providers.all_providers import HFProvider
    ...
    def enable_all_providers(self, env: dict):
        ...
        if env.get("DEEPSEEK_API_KEY"):
            from ultron.providers.extra_providers import DeepSeekProvider
        ...
        if env.get("ANTHROPIC_API_KEY"):
            from ultron.providers.extra_providers import AnthropicProvider
        ...
        if env.get("MISTRAL_API_KEY"):
            from ultron.providers.extra_providers import MistralProvider
        ...
        if env.get("COHERE_API_KEY"):
            from ultron.providers.extra_providers import CohereProvider
        ...
        if env.get("FIREWORKS_API_KEY"):
            from ultron.providers.extra_providers import FireworksProvider
        ...
        for provider_name in list(healthy):
            ...
            try:
                ...
                if 'tools' in params:
                    ...
                    from ultron.providers.base import Message
                    converted = []
                    for m in messages:
