...
class RPAOperatorAgent(Agent):
    agent_name = "RPAOperatorAgent"
    agent_description = "Autonomous computer use agent."
    ...
    async def _screenshot(self, task: Task) -> TaskResult:
        ...
        await self.store_context(f"screen_{task.task_id}", {
    ...
    async def _ocr_read(self, task: Task) -> TaskResult:
        ...
        if not screenshots:
            return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error="No screenshot found")

        filepath = screenshots[-1]
        reader = self._init_easyocr()
        if reader is None:
            return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error="EasyOCR could not be initialized.")
    ...
    async def _media_control(self, task: Task) -> TaskResult:
        ...
        return TaskResult(task_id=task.task_id, status=TaskStatus.SUCCESS, output=output)
    ...
    async def _launch_app(self, task: Task) -> TaskResult:
        ...
        app_name = task.context.get("app_name", "") or task.input_data
    ...
    async def _find_and_click_by_description(self, task: Task) -> TaskResult:
        ...
        prompt = (
            f"Find the exact X, Y coordinates for the following element: '{task.input_data}'\n"
        ...
        if json_match:
            ...
            return TaskResult(
                task_id=task.task_id,
                status=TaskStatus.SUCCESS,
                output=f"Found and clicked '{task.input_data}' at ({x}, {y})",
                tool_calls=[ToolCall(name="find_and_click", arguments={"description": task.input_data, "coords": [x, y]}, success=True)],
            )
        else:
            return TaskResult(
                task_id=task.task_id,
                status=TaskStatus.FAILED,
                error=f"Could not determine coordinates for: {task.input_data}",
            )
    ...
    async def _autonomous_action(self, task: Task) -> TaskResult:
        ...
        while step_idx < len(plan) and step_idx < max_steps:
            ...
            sub_task = Task(
                id=f"{task.task_id}_step_{step_idx}",
    ...
    async def _weather_report(self, task: Task) -> TaskResult:
        ...
        city = task.context.get("city", "") or task.input_data
        if not city:
            return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error="Şehir belirtilmedi.")
