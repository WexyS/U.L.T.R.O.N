...
class UltronVisionAgent(Agent):
    agent_name = "UltronVisionAgent"
    agent_description = "Integrated vision agent for desktop awareness."
    ...
    async def _subscribe_events(self) -> None:
        async def on_vision_request(event):
            ...
            await self._publish_event("vision_result", {"task_id": task.task_id, "output": result.output, "error": result.error})
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if not image_b64:
                return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error="No visual context available. Ensure Ultron Vision is running or dependencies are installed.")

            # 2. Process based on action
            if action in ["analyze", "capture", "process_screen"]:
                query = task.input_data or "Describe what is on the screen and identify key UI elements."
                ...
                try:
                    ...
                    return TaskResult(task_id=task.task_id, status=TaskStatus.SUCCESS, output=resp.content)
                ...
                return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error=f"Unknown action {action}")
