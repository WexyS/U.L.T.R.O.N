...
from ultron.core.browser_service import BrowserService
...
class ResearcherAgent(Agent):
    ...
    agent_description = "Deep research agent with multi-hop reasoning and citations."
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if task.intent == "architect" or any(kw in task.input_data.lower() for kw in ["mimari", "architect", "clone", "klonla"]):
                # Extract URL
                url_match = re.search(r'https?://\S+', task.input_data)
                if url_match:
            ...
            if any(kw in task.input_data.lower() for kw in ["çal", "scrape", "profil", "ekstrak", "veri topla"]):
                url_match = re.search(r'https?://\S+', task.input_data)
                if url_match:
                    target_url = url_match.group(0)
            ...
            if not search_results:
                if query.startswith("http"):
                    ...
                    return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error="No search results found after multiple attempts across all backends.")
    ...
    async def _search(self, query: str) -> list[dict]:
        ...
        if os.getenv("TAVILY_API_KEY"):
            from ultron.skills.tavily_search import search_tavily
