...
class CoderAgent(Agent):
    ...
    agent_description = "Autonomous software engineer agent."
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if not code:
                return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error="No code generated")
            ...
            if should_execute and self.allow_execution:
                ...
                return TaskResult(task_id=task.task_id, status=TaskStatus.SUCCESS, output=code)
        finally:
    ...
    async def _generate_code(self, task: Task) -> str:
        ...
        # Save to workspace
        filename = task.context.get("filename", f"task_{task.task_id[:8]}.{self._ext_for_language(language)}")
        filepath = self.work_dir / filename
        filepath.write_text(code, encoding="utf-8")

        await self.store_context(f"code_{task.task_id}", {"code": code, "filepath": str(filepath)})
        logger.info("Generated code for task %s → %s", task.task_id, filepath)
        return code
    ...
    async def _self_healing_loop(
        ...
        for iteration in range(max_iterations):
            ...
            if iteration < max_iterations - 1:
                ...
                code = await self._fix_code(code, f"{task.input_data}\n\nDIAGNOSTICS:\n{diagnostic_context}", error)
    ...
    async def _generate_project(self, task: Task) -> TaskResult:
        ...
        messages = self._build_messages(
            f"Project Request: {task.input_data}\n"
        ...
        try:
            ...
            if not json_match:
                return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error="Invalid project JSON from LLM")
            
            data = json.loads(json_match.group())
            files = data.get("files", [])
            
            project_root = self.work_dir / f"project_{task.task_id[:8]}"
